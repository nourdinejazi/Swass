...
export const sendPasswordResetEmail = async (email: string, token: string) => {
  ...
  await resend.emails.send({
    ...
    <p>Cordialement,<br>L'équipe du Starting Club de Nabeul</p>
    `,
  });
};
...
export const sendVerificationEmail = async (email: string, token: string) => {
  ...
  await resend.emails.send({
    ...
    <p>Nous sommes ravis de vous accueillir au sein du Starting Club de Nabeul! Pour garantir la sécurité de votre compte, veuillez suivre les étapes ci-dessous pour vérifier votre adresse e-mail.</p>

    <p>Cliquez sur le lien ci-dessous pour confirmer votre adresse e-mail:</p>
    <a href="${confirmLink}">Vérifier mon adresse e-mail</a>

    <p>Si vous n'avez pas récemment créé de compte sur le site du Starting Club de Nabeul, veuillez ignorer cet e-mail.</p>
